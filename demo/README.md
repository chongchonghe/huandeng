...
The deck *is* a browser, so video needs no pipeline at all: put an `.mp4` in `attach/` and write a `<video>` tag.
...
uv run python tools/build-slides.py demo --check
```
...
So it is a different deck built from the same Markdown, not an exporter for this one. If you want slides built by Typst, use Touying directly — writing them by hand gives you its full API rather than whatever survives a Markdown round trip.
...
Reveal's `?print-pdf` mode wraps each slide in a `.pdf-page` whose height is a whole number of printed pages. A slide that holds too much comes out **two pages tall — the title alone on one page, the body on the next** — and nothing warns you. `--pdf` measures those boxes before printing and names any slide that spills:
