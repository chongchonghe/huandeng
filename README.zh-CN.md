...
**用纯文本写学术幻灯片——基于 [Touying](https://github.com/touying-typ/touying) 和 [Typst](https://typst.app)，或者 [Quarto](https://quarto.org/) 和 [reveal.js](https://revealjs.com/)，并自带 AI 技能，让大模型帮你改稿，但方向盘始终在你手里。**

[English](README.md)

一份源文件，同时导出三种格式：现场用的 PDF、可以直接发给别人的单文件 HTML、以及某些会议非要不可的 PPTX。视频在三种格式里都能播。还有一个检查工具，帮你拦住那些本来会带上讲台的错误。

**两套并行，地位相同。** 下面讲的都是 **Typst** 那套；**Quarto + reveal.js** 那套在 [`quarto/`](quarto/)，有[自己的说明](quarto/README.md)。它们是兄弟关系，不是新旧版本——每份报告各选一套，[一张表](quarto/README.md#which-flavour-should-i-use)说明什么场合选哪个更省事。
...
- **`quarto-deck`**——Quarto 那套的同一件事：本地的主题类，以及 reveal.js 和 Quarto 里那些不会报错的坑。
...
```
tools/build-slides.py   Typst 工具链——所有幻灯片共用，但不会被复制进任何一份
template/               Typst 起点。复制它，不要直接改。
demo/                   Typst 所有功能的可运行参考
quarto/                 Quarto + reveal.js 那套：自带 tools/、template/、demo/ 和说明
talks/                  你自己的，两套都放这里——已 gitignore，你的幻灯片不会进这个仓库
.agents/skills/         AI 技能（.claude/skills 是指向它的软链接）
```

一份 Typst 幻灯片就是 `main.typ` + `globals.typ` + `content.typ` + `attach/`，本身不含任何构建脚本。一份 Quarto 幻灯片是 `talk.qmd` + `_quarto.yml` + `theme.scss` + `attach/`，原则相同。

在根目录跑 `make check`，两套的每一份幻灯片都会被检查一遍。

## 一个值得说明的设计取舍

**每份幻灯片都是自包含的，这种重复是故意的。** `globals.typ`、`main.typ`（Quarto 那套则是 `theme.scss`、`_quarto.yml`、`fonts.html`）和字体都是*副本*，不是 import。任何一份幻灯片都不读取自己目录以外的东西。
...
基于 touying-typ 作者们的 [Touying](https://github.com/touying-typ/touying) 和 [Typst](https://typst.app)；另一套基于 [Quarto](https://quarto.org/) 和 [reveal.js](https://revealjs.com/)。字体为 [Fira Sans](https://github.com/mozilla/Fira) 和 [Fira Math](https://github.com/firamath/firamath)，SIL OFL 授权。
