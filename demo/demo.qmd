...
deck, `theme.scss` holds the look, `head.html` ships the typeface, and
...
::: {.highlight}
Do not start a fenced div with a heading. Pandoc writes that div out as a
`<section>`, and reveal's slide selector is `.slides section` — a descendant
selector — so it silently becomes a slide of its own. `make check` catches it.
:::
